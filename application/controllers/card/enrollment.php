...
class Enrollment extends CI_Controller {
	...
	function index()
	{
		...
		foreach ($cardType as $row)
		{
			...
			$data['cardType'] .= '<option value="'. $val .'" defaultallows="'. $defAllows .'" format="'.$format.'">'. $desc .'</option>';
		}
		$format = $firstformat;
	...
	function submit()
	{
		...
		$xml = !$this->coreencrypt ? '<CARDNO>'. $cardNo .'</>' : NULL;
		$xml .= '<ATMLOCK>'. $atmLock .'</>'.
			'<POSLOCK>'. $posLock .'</>'.
			'<WEBLOCK>'. $webLock .'</>'.
			'<CELLLOCK>'. $cellLck .'</>';
		...
		if ($row['errno'] > 0) {
			$success = FALSE;
			$message = isset($row['errmsg']) ? $row['errmsg'].':'.$row['errno'] : 'An error has occured.';
		} else {
			$success = TRUE;
