...
class ATM extends CI_Controller {
	...
	function index()
	{
		...
		$data['infoTerminalLuno']  = $row['luno'];
		...
		$data['infoTerminalLoc']   = ucwords(strtolower($row['location']));
		...
		$data['infoLastCommand']   = $row['cmdstat'] ? str_replace('Atm', 'ATM', ucwords(strtolower($row['cmdstat']))) : 'Unknown';
		$data['infoCommandStatus'] = $row['cmddesc'];
		...
		$this->load->view('monitoring/atm', $data);
	}
