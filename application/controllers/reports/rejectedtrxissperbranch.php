...
class RejectedTrxIssPerBranch extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getRejectedSumIssuerPerBranch($reportLog, $branchCode, $termType, $finswitch, $dtFrom, $dtTo);
		...
		$pdf->SetTitle('Rejected Transactions, Issuer Per Branch Summary');
		$pdf->SetSubject('');
		...
		<h2>'.$instName.'</h2>
		<h1>REJECTED TRANSACTIONS, ISSUER PER BRANCH SUMMARY</h1>
		<h3>'. $branchName .'</h3>
		...
		$pdf->Output('rejectedtrxissperbranch_' . $dateSaveFormat . '.pdf', 'I');
	}
}
