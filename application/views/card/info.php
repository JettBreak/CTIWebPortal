...
<div id="cardInfo">
    ...
    <ul class="tabs fullTabs">
        ...
        <li><a href="#emvinfo" tableIndex="3">EMV Info</a></li>
    </ul>
    </ul>
    <div class="tab_container">
        <div id="info" class="tab_content">
            <table width="100%" style="margin-top:0px">
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <select name="acctType" id="acctType" style="width:212px" <?php echo html_entity_decode($readonly); ?> >
                ...
                <tr>
                    ...
                    <td colspan="3"><input name="embossName" id="embossName" type="text" style="width:277px" maxlength="20" value="<?php echo $embossName; ?>" readonly/></td>
                </tr>
                <tr>
        ...
        <div id="emvinfo" class="tab_content">
                ...
                <fieldset>
                    ...
                    <table>     
                        <tr>
                            <td width=250>
                                <input type="checkbox" name="trnTrack2" id="trnTrack2" value="1" <?php echo $trnTrack2; ?> /><label for="trnTrack2">Use Track2</label>
                            </td>
                            <td>
                                <input type="checkbox" name="trnExpr" id="trnExpr" value="1" <?php echo $trnExpr; ?> ><label for="trnExpr">Check Expiration</label>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <input type="checkbox" name="trnICCT2" id="trnICCT2" value="1" <?php echo $trnICCT2; ?> /><label for="trnICCT2">Use ICC</label>
                            </td>
                            <td>
                                <input type="checkbox" name="trnCVV" id="trnCVV" value="1" <?php echo $trnCVV; ?> /><label for="trnCVV">Check CVV</label>
                                <span style="margin-left:20px;"><input type="number" name="servcCVV" id="servcCVV" max="999" min="0" maxlength="3" step="1" style="width:50px; text-align:center" value="<?php echo $servCVV; ?>"/></span>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <input type="checkbox" name="trnARQC" id="trnARQC" value="1" <?php echo $trnARQC; ?> /><label for="trnARQC">Verify ARQC</label>
                            </td>
                            <td>
                                <input type="checkbox" name="trnICVV" id="trnICVV" value="1" <?php echo $trnICVV; ?> /><label for="trnICVV">Check iCVV</label>
                                <span style="margin-left:18px;"><input type="number" name="servcICVV" id="servcICVV" max="999" min="0" maxlength="3" step="1" style="width:50px; text-align:center" value="<?php echo $servICVV; ?>" /></span>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <input type="checkbox" name="trnARPC" id="trnARPC" value="1" <?php echo $trnARPC; ?> /><label for="trnARPC">Generate ARPC</label>
                            </td>
                            <td>
                                <input type="checkbox" name="trnCntr" id="trnCntr" value="1" <?php echo $trnCntr; ?> /><label for="trnCntr">Send Transaction Counters</label>
                            </td>
                        </tr>                                                                                                                                                                                                                                                                                                                     
                    </table>
                </fieldset>
...
<div id="bottom">
    <span class="buttons floatLeft">
        <?php echo html_entity_decode($updatebtn); ?>
        <?php echo html_entity_decode($searchbtn); ?>
        <?php echo html_entity_decode($browsebtn); ?>
        <?php echo html_entity_decode($modifybtn); ?>
        <!--<button id="closeCardBtn">Close Card</button>--><?php echo html_entity_decode($resetLimitsBtn); ?>
        <?php echo html_entity_decode($emvButton); ?>
    </span>
    <span class="buttons floatRight">
        <?php echo html_entity_decode($cardhist); ?>
        <?php echo html_entity_decode($resetbtn); ?>
        <!--<button id="refreshBtn" class="hidden">Refresh</button
        >-->
        <?php echo html_entity_decode($closebtn); ?>
    </span>
</div>
</form>
    ...
    <div class="top">
        ...
        <div class="dataTables_custom floatLeft">
            <span class="info">
                ...
                <select id="limitID" style="width:150px" <?php echo html_entity_decode($readonly); ?>>
...
$(function () {
        ...
        if (!$(this).hasClass('active')) {
            ...
            switch (tab) {
                case '#info':   
                    ...
                    $(modifyBtn + ',' + emvButton + ',' + resetLimitsBtn).hide();
                    break;
                case '#accounts':
                    $(updateBtn + ',' + emvButton + ',' + searchBtn + ',' + browseBtn + ',' + resetBtn + ',' + modifyBtn + ',' + resetLimitsBtn).hide();
                    break;
                case '#bills':
                    $(updateBtn + ',' + emvButton + ',' + searchBtn + ',' + browseBtn + ',' + resetBtn + ',' + modifyBtn + ',' + resetLimitsBtn).hide();
                    break;
                case '#limits':
                    $(searchBtn + ',' + emvButton + ',' + browseBtn).hide();
                    //$(updateBtn + ',' + modifyBtn + ',' + resetLimitsBtn + ',' + refreshBtn).show();
                    $(updateBtn + ',' + modifyBtn + ',' + refreshBtn + ',' + resetLimitsBtn).show();
                    break;
                case '#emvinfo':
    ...
    $('#cardhist').click(function (e) {
        /*e.preventDefault();
        var data = {
            prseqno: <?php echo $prseqno; ?>
        }
        exportData(data);*/
        window.location.href = 'card/info/getCardHistory?pr='+<?php echo $prseqno; ?>+'&br='+<?php echo $brseqno; ?>;
        return false;
    });
    ...
    //old reset PIN retry count button
    ...
    $(resetLimitsBtn).click(function (e) {
        var buttons = {
            Save: function () {
                if ($('#trxTable').find('input:checked').length > 0) {
                    messageBoxV2('Proceed with current settings?', 'Confirm', {
                        OK: function () {
                            $('#resetLimitsForm').submit();
                        },
                        Cancel: function () {
                            $(this).dialog('close');
                            //return false;
        ...
        modalDialog('card/resetlimits?' + params, 'RESET LIMITS', buttons, 400);
        ...
        messageBoxV2('Proceed with current settings?', 'Confirm', {
            OK: function () {
                requests.push(
                    $.ajax({
                        type: 'POST',
                        url: 'card/info/updateemvcard',
                        data: {
                            prq: '<?php echo $prseqno; ?>',
                            tk2: $('#trnTrack2').attr('checked') ? 1 : 0,
                            icc: $('#trnICCT2').attr('checked') ? 1 : 0,
                            icv: $('#trnICVV').attr('checked') ? 1 : 0,
                            icd: $('#servcICVV').val(),
                            cvv: $('#trnCVV').attr('checked') ? 1 : 0,
                            cvd: $('#servcCVV').val(),
                            arq: $('#trnARQC').attr('checked') ? 1 : 0,
                            arp: $('#trnARPC').attr('checked') ? 1 : 0,
                            exp: $('#trnExpr').attr('checked') ? 1 : 0,
                            txc: $('#trnCntr').attr('checked') ? 1 : 0

                        },
                        dataType: 'json',
                        beforeSend: function () {
                            waitMessage('Updating card EMV settings...');
                        },
                        success: function (data) {
                            if (data.success === true) {
                                messageBox(data['message']);
   ...
   $('#clbtn').click(function() {
        window.location.hash = 'card/cardissuanceapproval';
        return false;
   });
    ...
    if (<?php echo $ronly; ?> != '1') {
        ...
        $('#embossName').removeAttr('readonly');
    }
     ...
     function exportData(data)
    {
        ...
        requests.push(
            jqxhr = $.ajax({
                type: 'POST',
                url: 'card/info/getCardHistory',
                data: data,
                dataType: 'json',
                beforeSend: function () {
                    waitMessage('Exporting Transactions History...');
                },
                complete: function () {
                    $(MSGBOX).dialog('close');
                }
