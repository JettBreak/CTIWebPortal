...
class BillsPaymentRep extends CI_Controller { 
  ...
  function process()
  {
    ...
    foreach ($stDPS as $row) {
      ...
      //CARD
      $stCard = trim(substr($row['cardx'],0,18));
      ...
      if (!$stSubNo) {
        ...
        $stSubNo = $core->subsnoDecrypt($stSubNo, "thequickbrownfoxjumpsoverthelazydog");
      ...
      // stMODE rules
      $stNode = $row['chname'];
      if($termType == 'ATM' && $stNode === $onusCode) {
        $stMODE = 1;  
      } elseif ($termType == 'POS') {
        $stMODE = 7;  
      } else {
    ...
    $data = array(
      $bnkMnem . $suffix . $ext1 => $BBPRDPS,
      $bnkMnem . $suffix . $ext2 => $BBPRSUM
