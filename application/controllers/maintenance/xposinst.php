...
class XPOSINST extends CI_Controller {
	...
	function index()
	{
		...
		$data['header'] = 'New Institution Entry';
		$data['termCodeParams'] = 'class="validate[required] numbersOnly" maxlength="8"';
		$data['termID'] = NULL;
		$data['desc'] = NULL;
		$data['mercID'] = NULL;
		$data['termIDParams'] = 'class="validate[required]"';
		$data['submitBtnVal'] = 'maintenance/xposinst/submit';
		$data['waitMsg'] = 'Sending new Institution entry...';
		$data['submitBtnMsg'] = 'Submit new Institution entry?';
	...
	function submit()
	{
		...
		if ($row['errno'] !== '8') { //if session valid
			...
			$row = $this->pos_model->insertPartnerInstitution(
