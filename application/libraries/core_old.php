			...
			if (substr($userAllows, $pos - 1, 1) === '1') {
				...
				if ( $pos === REPORTS_NO ) {
					foreach	( $sub['subMenu'] as $PDFRep => $rep ) {
						...
						if ( !in_array($PDFRep, array(REPBRANCHLOG_NO, BARTSFILE_NO, BILLSPAYMENTREP_NO)) ) {
				...
				foreach ($sub['subMenu'] as $posx => $submenu) {
					...
					if (
						...
						( $pos === REPORTS_NO && substr($userAllows, $posx - 1, 1) === '1' 
						&& in_array($posx, array(REPBRANCHLOG_NO, BARTSFILE_NO, BILLSPAYMENTREP_NO)) ) || 
		...
		foreach ($menu as $pos => $sub) {	
			if (substr($userAllows, $pos - 1, 1) === '1') {//$pos == 222 for testing
				...
				foreach ($sub['subMenu'] as $posx => $submenu) {
					if (
						( $pos !== REPORTS_NO && substr($userAllows, $posx - 1, 1) === '1' ) || 
						( $pos === REPORTS_NO && in_array($posx, array(PDFGEN_NO, REPBRANCHLOG_NO, BARTSFILE_NO, BILLSPAYMENTREP_NO)) )
					)
					{
