...
class POSLoanPayReport extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getPOSLoanPayment($reportLog, $branchCode, $onusCode, $dtFrom, $dtTo);
		...
		$pdf->SetTitle('POS Loan Payment, ONUS Report');
		$pdf->SetSubject(NULL);
		...
		foreach ($data as $key => $d) {
			...
			<h2>'.$instName.'</h2>
			<h1>POS LOAN PAYMENT, ONUS REPORT : '.$key.'</h1>
			<h3>'. $branchName .'</h3>
