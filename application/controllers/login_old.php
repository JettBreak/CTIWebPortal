...
class Login extends CI_Controller {
	...
	function submit()
	{
		...
		$sessionID = $core->encrypt($userID, session_id());
		...
		$isTeller = $userID === 'coreware' ? TRUE : FALSE;
		...
		if ( in_array($errNo, array('2', '1')) ) {				
			...
			switch ($this->inst['theme']) {
				case 1:
					...
					$floatMenu = $core->getFloatingMenu($userAllows, $grpseqno);
				...
				case 2:
					$header = '<tr>
							<td id="instName" rowspan="2" style="vertical-align:middle !important">'. $this->inst['bankName'] .'</td>
							<td colspan="2">
								'. $core->getFloatingMenux($userAllows, $grpseqno) .'     
							</td>
							<td id="topRight">'. $topRight .'</td>
						</tr>';
			...
			if ($isTeller) {
				$header = '<tr>
					<td id="instName" rowspan="2" style="vertical-align:middle !important; font-size: 30px;">'. $this->inst['bankName'] .'</td>
					<td style="font-size: 15px;"><strong>Teller Name:</strong> '.$userName.'</td>
