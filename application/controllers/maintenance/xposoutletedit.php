...
class XPOSOutletEdit extends CI_Controller {
	...
	function index($outletid)
	{
		...
		$result = $this->pos_model->getOutletInfo($outletid);
		
		$data['outletid'] = isset($result['outletid']) ? $result['outletid'] : '';
		$data['outletseqno'] = isset($result['outletseqno']) ? $result['outletseqno'] : '';
		$data['outletname'] = isset($result['outletname']) ? $result['outletname'] : '';
...
/*				if (!$this->core->isHeadOffice() && $row['brseqno'] === $this->core->getBranchID()) {
			...
			$data['institutions'] = '<option value="">No Outlet Defined</option>';
		}
		//end
		...
		$data['header'] = 'Update Outlet Entry ';
		$data['termCodeParams'] = 'class="validate[required] numbersOnly" maxlength="8"';
		$data['termID'] = NULL;
		$data['desc'] = NULL;
		$data['mercID'] = NULL;
		$data['termIDParams'] = 'class="validate[required]"';
		$data['submitBtnVal'] = 'maintenance/xposoutletedit/submit';
		$data['waitMsg'] = 'Updating Outlet entry...';
		$data['submitBtnMsg'] = 'Update Outlet entry?';
	...
	function submit()
	{
		...
		if ($row['errno'] !== '8') { //if session valid
			$outletseqno = $input->post('outletseqno', TRUE);
			...
			$row = $this->pos_model->updateOutlet(
				$outletseqno,$outletname,$instid
			);
