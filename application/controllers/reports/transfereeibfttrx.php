...
class TransfereeIBFTTrx extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getIBFTTransferee($reportLog, $branchCode, $termType, $dtFrom, $dtTo);
		...
		$totalIBFT = array (
			'ibfttrsaApprvSubTotal' => 0,
			'ibfttrsaRvrslSubTotal' => 0,
			'ibfttrsaRejctSubTotal' => 0,
			'ibfttrcaApprvSubTotal' => 0,
			'ibfttrcaRvrslSubTotal' => 0,
			'ibfttrcaRejctSubTotal' => 0
			);
		...
		foreach ($tpt as $row) {
			...
			$content = '<tr>
				...
				<td width="250">'. substr($row['prkey1'], 0, 6).'- '.substr($row['prkey1'], 6).'</td>
				<td width="120" align="center">'. $row['issname'] .'</td>
				<td width="250" '. $a1Align .'>'. substr($acct1, -14) .'</td>
				<td width="60">'. $acctDesc .'</td>
				<td width="60">'. $row['msgtype'] .'</td>	
				<td width="160">'. $mnemonic .'</td>	
				<td width="120">'. $xml->getVALUE('ACQCODE_MNEM') .'</td>	
				<td width="250" '. $a2Align .'>'. substr($acct2, -14) .'</td>	
				<td width="80" align="right">'. $row['sysvcode'] .'</td>	
			...
			switch ($row['msgtype']) {
				...
				case 52:
					switch ($mnemonic) {
						case 'IBFTTRSA':
							$data[$row['termcode']]['ibfttrsaApprv'] += $amtReq;
							$totalIBFT['ibfttrsaApprvSubTotal'] += $amtReq;
							break;
						case 'IBFTTRCA':
							$data[$row['termcode']]['ibfttrcaApprv'] += $amtReq;
							$totalIBFT['ibfttrcaApprvSubTotal'] += $amtReq;
							break;
					}
					break;
				case 53:
					switch ($mnemonic) {
						case 'IBFTTRSA':
							$data[$row['termcode']]['ibfttrsaRejct'] += $amtReq;
							$totalIBFT['ibfttrsaRejctSubTotal'] += $amtReq;
							break;
						case 'IBFTTRCA':
							$data[$row['termcode']]['ibfttrcaRejct'] += $amtReq;
							$totalIBFT['ibfttrcaRejctSubTotal'] += $amtReq;
							break;
							}
					break;
				case 71:
					switch ($mnemonic) {
						case 'IBFTTRSA':
							$data[$row['termcode']]['ibfttrsaRvrsl'] += $amtReq;
							$totalIBFT['ibfttrsaRvrslSubTotal'] += $amtReq;
							break;
						case 'IBFTTRCA':
							$data[$row['termcode']]['ibfttrcaRvrsl'] += $amtReq;
							$totalIBFT['ibfttrcaRvrslSubTotal'] += $amtReq;
							break;
					}
					break;
				}
		...
		$pdf->SetTitle('Transferee IBFT Transaction');
		$pdf->SetSubject(NULL);
		...
		foreach ($data as $key => $d) {
			...
			<h2>'.$instName.'</h2>
			<h1>TRANSFEREE IBFT TRANSACTION: '. $key .' OTHER BANK TERMINAL</h1>
			<h3>'. $branchName .'</h3>
			...
			<thead>
				<tr>
					<th align="center">&nbsp;</th>
					<th align="center">Apprvd IBFTTRSA</th>
					<th align="center">Apprvd IBFTTRCA</th>
					<th align="center">IBFTTRSA Revrsl</th>
					<th align="center">IBFTTRCA Revrsl</th>
					<th align="center">Rejctd IBFTTRSA</th>
					<th align="center">Rejctd IBFTTRCA</th>
				</tr>
			</thead>
			<tbody>
				<tr align="right">
					<td><b>TOTAL:</b></td>
					<td>'. $core->currency($d['ibfttrsaApprv']) .'</td>
					<td>'. $core->currency($d['ibfttrcaApprv']) .'</td>
					<td>'. $core->currency($d['ibfttrsaRvrsl']) .'</td>
					<td>'. $core->currency($d['ibfttrcaRvrsl']) .'</td>
					<td>'. $core->currency($d['ibfttrsaRejct']) .'</td>
					<td>'. $core->currency($d['ibfttrcaRejct']) .'</td>
				</tr>
			</tbody>
			</table>';
		...
		$pdf->Output('transfereeibfttrx_' .$dateSaveFormat . '.pdf', 'I');
	}
}
/* End of file transfereeibfttrx.php */
/* Location: ./application/reports/transfereeibfttrx.php */