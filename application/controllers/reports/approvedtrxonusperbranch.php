...
class ApprovedTrxOnusPerBranch extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getSumOnUsPerBranch($reportLog, $branchCode, $termType, 'ATM', $onusCode, $dtFrom, $dtTo);
		...
		$pdf->SetTitle('Approved Transactions, Onus Per Branch Summary');
		$pdf->SetSubject('');
		...
		<h2>'.$instName.'</h2>
		<h1>APPROVED TRANSACTIONS, ONUS PER BRANCH SUMMARY</h1>
		<h3>'. $branchName .'</h3>
		...
		$pdf->Output('approvedtrxonusperbranch_' . $dateSaveFormat . '.pdf', 'I');
	}
}
/* End of file approvedtrxonusperbranch.php */
/* Location: ./application/reports/approvedtrxonusperbranch.php */