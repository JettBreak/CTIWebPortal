...
class ApprovedMobileBnkng extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getOurRChatOtherTerminal($reportLog, $branchCode, $termType, $onusCode, $dtFrom, $dtTo);
		...
		foreach ($tpt as $row) {
			...
			$content = '<tr>
				...
				<td width="140" align="center">'. $acqCode .'</td>
		...
		$pdf->SetTitle('Approved Mobile Banking Transactions');
		$pdf->SetSubject(NULL);
		...
		foreach ($data as $key => $d) {
			...
			<h2>'.$instName.'</h2>
			<h1>APPROVED MOBILE BANKING TRANSACTIONS: '.$key.'</h1>
			<h3>'. $branchName .'</h3>
			...
			<thead>
				<tr>
					...
					<th width="140" align="center">ACQ</th>
					<th width="100">TC</th>
		...
		$pdf->Output('approvedcardholderatotherterm_' .$dateSaveFormat . '.pdf', 'I');
	}
}