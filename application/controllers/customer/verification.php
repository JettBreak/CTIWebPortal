...
class Verification extends CI_Controller {
	...
	function index()
	{
		...
		$this->load->view('customer/verification', $data);
	}
	...
	function getData()
	{
		...
		if ($brseqno == -1) {
			$result = $this->customer_model->getCustomerForApprovalHO();
		} else {
			$result = $this->customer_model->getCustomerForApproval($brseqno);
		}
	...
	function reject()
	{
		...
		*/
		...
		foreach ($customers as $custinfo) {
			...
			$result = $customer->removeRejectedCustomer(
				$custkey,
				$ipAddress,
				$workstation,
				$cifseqno,
	...
	function verify()
	{
		...
		*/
		...
		foreach ($customers as $custinfo) {
			...
			$result = $customer->insertApprovedCustomer(
				$custkey,
				$ipAddress,
				$workstation,
				$cifseqno,
