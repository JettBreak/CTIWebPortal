...
class Verify extends CI_Controller {
	...
	function submit($type)
	{
		...
		switch ($type) {
			case 'replace':
				$result = $card->getValidCardForReplacement(($core->isCoreEncrypt() ? $tokenid : $cardBIN),$core->isCoreEncrypt());
				break;
			case 'change':
				$result = $card->getValidCardForChangeStat($cardBIN);
				break;
			default:
				$result = $card->getCardInfo(($core->isCoreEncrypt() ? $tokenid : $cardBIN), $branchID, $ipAddress, $workstation, $userAudit);
				break;
		}
