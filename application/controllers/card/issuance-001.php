...
class Issuance extends CI_Controller {
	...
	function index()
	{
		...
		if ($cust = $this->cache->get($this->core->getSessionID() . 'cust')) {
			...
			$data['custName'] = utf8_decode($cust['fullName']);
	...
	{	
		...
		$result = $card->getValidCardForActivation($tokenid);
		$row = $result->row_array();
		...
		$result = $card->searchAccountbyCIF($cifseqno, $row['brseqno']);
	...
	function submit()
	{
		...
		if ($core->isISOCustomer()) {
			...
			foreach ($data as $acctbycif) {
				...
				$cifacctno		= substr($acctbycif, 0, 12);
				$cifaccttype	= substr($acctbycif, 13, 2);
				$cifacctprseqno	= substr($acctbycif, 16);
