			...
			messageBox('Reset password for User ID <strong>['+ user.ID +']</strong>?','Confirm','confirm',function() {
				requests.push(
					$.ajax({
						...
						success: function(data) {
							if (data['resetPwd'] === true) { //User ID <strong>['+ user.ID +']</strong> p
								messageBox('Password was successfully reset.<br>System generated password has been emailed to the user.');
								oTable.fnUpdate(data['newStat'], oTable.fnGetPosition($('#' + user.ID)[0]), 4, false);
							} else {
								messageBox('Unable to reset password: '+data.newStat);
							}
						}
					})
				);
