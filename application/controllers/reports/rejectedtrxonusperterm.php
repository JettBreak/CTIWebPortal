...
class RejectedTrxOnusPerTerm extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getRejectedSumOnUsPerTerminal($reportLog, $branchCode, $onusCode, $dtFrom, $dtTo);
		//
		...
		$pdf->SetTitle('Rejected Transactions, Onus Per Terminal Summary');
		$pdf->SetSubject('');
		...
		<h2>'.$instName.'</h2>
		<h1>REJECTED TRANSACTIONS, ONUS PER TERMINAL SUMMARY</h1>
		<h3>'. $branchName .'</h3>
		...
		$pdf->Output('rejectedtrxonusperterm_' . $dateSaveFormat . '.pdf', 'I');
	}
}
/* End of file rejectedtrxonusperterm_.php */
/* Location: ./application/reports/rejectedtrxonusperterm_.php */