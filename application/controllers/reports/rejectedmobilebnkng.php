...
class RejectedMobileBnkng extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getRejectedOurRChatOtherTerminal($reportLog, $branchCode, $termType, $onusCode, $dtFrom, $dtTo);
		...
		if ($result->num_rows() === 0 || count($data) === 0) {
			$data[' '] = array(
		...
		foreach ($tpt as $row) {
				...
				case 'IBFT':
					switch ($mnemonic) {
						...
						case 'IBFTBTCA':
							...
							if ($mnemonic === 'IBFTTBTSA') {
								$mnemonic = 'IBFTSA';	
							} else if ($mnemonic === 'IBFTTBTCA'){
								$mnemonic = 'IBFTCA';
							}
							break;
						case 'IBFTWDSA':
			...
			$content = '<tr>
				...
				<td width="400" '. $rAlign .'>'. $remarks .'</td>
				<td width="180" align="right">'. $core->currency($amtReq) .'</td>
				<td width="180" align="right">'. $core->currency($amtAth) .'</td>
			</tr>';
		...
		$pdf->SetTitle('Rejected Mobile Banking Transactions');
		$pdf->SetSubject(NULL);
		...
		foreach ($data as $key => $d) {
			...
			<h2>'.$instName.'</h2>
			<h1>REJECTED MOBILE BANKING TRANSACTIONS: '.$key.'</h1>
			<h3>'. $branchName .'</h3>
			<h4>Transaction Date(s): '. $reportDate .'</h4>
			<table cellspacing="10" width="100%">
			<thead>
				<tr>
					<th width="150">LOGKEY</th>
					...
					<th width="60">TY</th>
					<th width="90">VCD</th>
					<th width="400" align="center">REMARKS</th>
		...
		$pdf->Output('rejectedcardholderatotherterm_' .$dateSaveFormat . '.pdf', 'I');
	}
}