  ...
  function index()
  {
    ...
    if ($numRows > 0) {
      foreach ($resultArr as $row) {
        ...
        $data['location'] .= '<option value="'. $row['loccode'] .'" areaname="'. $area .'" brname="'. $branch .'">'. substr($row['location'], 0, 25) .'</option>';
  ...
  function submit()
  {
    ...
    if ($row['errno'] !== '8') { //if session valid
      ...
      if (isset($row['errno']) && (int) $row['errno'] === 0) {
        ...
        $auditXML .= '<old></><new></><field>New ATM</><details>Termcode: '.$termCode.'-'.$description.'</>';
