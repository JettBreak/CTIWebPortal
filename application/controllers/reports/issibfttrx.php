...
class IssIBFTTrx extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getIBFTIssuerTransaction($reportLog, $branchCode, $termType, $finswitch, $bnkcode, $dtFrom, $dtTo);
		...
		$totalIBFT = array (
			'ibftwdsaApprvSubTotal' => 0,
			'ibftwdsaRvrslSubTotal' => 0,
			'ibftwdsaRejctSubTotal' => 0,
			'ibftwdcaApprvSubTotal' => 0,
			'ibftwdcaRvrslSubTotal' => 0,
			'ibftwdcaRejctSubTotal' => 0
			);
		...
		foreach ($tpt as $row) {
			...
			$acqmnem =  $xml->getVALUE('ACQCODE_MNEM');
			if ($acqmnem === '' || $acqmnem === 'null' || $acqmnem === NULL) {
				$acqmnem = '-';	
			}
			...
			$content = '<tr>
				<td width="180">'. $logkey .'</td>
				<td width="170">'. $traceNo.'</td>
				<td width="220">'. $sequenceNo .'</td>
				<td width="150">'. $core->formatDate('H:i:s', $row['dtlog']) .'</td>
				<td width="150">'. $core->formatDate('m/d/y', $row['dtlog']) .'</td>
				<td width="140">'. $row['tpluno'] .'</td>
				<td width="350">'. substr($row['prkey1'], 0, 6).'- '.substr($row['prkey1'], 6).'</td>
				<td width="140" align="center">'. $row['issname'] .'</td>
				<td width="350" '. $a1Align .'>'. substr($acct1, -14) .'</td>
				<td width="80">'. $acctDesc .'</td>
				<td width="80">'. $row['msgtype'] .'</td>	
				<td width="170">'. $mnemonic .'</td>	
				<td width="140" align="center">'. $acqmnem .'</td>	
				<td width="350" '. $a2Align .'>'. substr($acct2, -14) .'</td>	
				<td width="100">'. $row['sysvcode'] .'</td>	
				<td width="380" '. $rAlign .'>'. $remarks .'</td>		
				<td width="170" align="right">'. $core->currency($amtReq) .'</td>
				<td width="170" align="right">'. $amtAth .'</td>
			</tr>';
			...
			switch ($row['msgtype']) {
				...
				case 52:
					switch ($mnemonic) {
						case 'IBFTWDSA':
							$data[$row['termcode']]['ibftwdsaApprv'] += $amtReq;
							$totalIBFT['ibftwdsaApprvSubTotal'] += $amtReq;
							break;
						case 'IBFTWDCA':
							$data[$row['termcode']]['ibftwdcaApprv'] += $amtReq;
							$totalIBFT['ibftwdcaApprvSubTotal'] += $amtReq;
							break;
					}
					break;
				case 53:
					switch ($mnemonic) {
						case 'IBFTWDSA':
							$data[$row['termcode']]['ibftwdsaRejct'] += $amtReq;
							$totalIBFT['ibftwdsaRejctSubTotal'] += $amtReq;
							break;
						case 'IBFTWDCA':
							$data[$row['termcode']]['ibftwdcaRejct'] += $amtReq;
							$totalIBFT['ibftwdcaRejctSubTotal'] += $amtReq;
							break;
							}
					break;
				case 71:
					switch ($mnemonic) {
						case 'IBFTWDSA':
							$data[$row['termcode']]['ibftwdsaRvrsl'] += $amtReq;
							$totalIBFT['ibftwdsaRvrslSubTotal'] += $amtReq;
							break;
						case 'IBFTWDCA':
							$data[$row['termcode']]['ibftwdcaRvrsl'] += $amtReq;
							$totalIBFT['ibftwdcaRvrslSubTotal'] += $amtReq;
							break;
					}
					break;
				}
		...
		$pdf->SetTitle('Issuer IBFT Transaction');
		$pdf->SetSubject(NULL);
		...
		foreach ($data as $key => $d) {
			...
			<h2>'.$instName.'</h2>
			<h1>ISSUER IBFT TRANSACTION: '. $key .' OTHER BANK TERMINAL</h1>
			<h3>'. $branchName .'</h3>
			<h4>Transaction Date(s): '. $reportDate .'</h4>
			<table cellspacing="10" width="100%">
			<thead>
				<tr>
					<th width="180">LOGKEY</th>
					<th width="170">TRACE NO.</th>
					<th width="220">SEQUENCE NO.</th>
					<th width="150">TIME</th>
					<th width="150">DATE</th>
					<th width="140">LUNO</th>
					<th width="350">BANK ID / CARD #</th>
					<th width="140" align="center">ISSUER</th>
					<th width="350">FROM ACCOUNT</th>
					<th width="80">TY</th>
					<th width="80">TC</th>
					<th width="170">TRXCODE</th>
					<th width="140" align="center">BANK</th>
					<th width="350">TO ACCOUNT</th>
					<th width="100">VCD</th>
					<th width="380" align="center">REMARKS</th>
					<th width="170">TRX AMT</th>
					<th width="170">AMT AUTH</th>
				</tr>
			</thead>
			<tbody>
				'. $d['content'] .'
			...
			<thead>
				<tr>
					<th align="center">&nbsp;</th>
					<th align="center">Apprvd IBFTWDLSA</th>
					<th align="center">Apprvd IBFTWDLCA</th>
					<th align="center">IBFTWDLSA Revrsl</th>
					<th align="center">IBFTWDLCA Revrsl</th>
					<th align="center">Rejctd IBFTWDLSA</th>
					<th align="center">Rejctd IBFTWDLCA</th>
				</tr>
			</thead>
			<tbody>
				<tr align="right">
					<td><b>TOTAL:</b></td>
					<td>'. $core->currency($d['ibftwdsaApprv']) .'</td>
					<td>'. $core->currency($d['ibftwdcaApprv']) .'</td>
					<td>'. $core->currency($d['ibftwdsaRvrsl']) .'</td>
					<td>'. $core->currency($d['ibftwdcaRvrsl']) .'</td>
					<td>'. $core->currency($d['ibftwdsaRejct']) .'</td>
					<td>'. $core->currency($d['ibftwdcaRejct']) .'</td>
				</tr>
			</tbody>
			</table>';
		...
		$pdf->Output('issibfttrx_' .$dateSaveFormat . '.pdf', 'I');
	}
}
/* End of file issibfttrx.php */
/* Location: ./application/reports/issibfttrx.php getibfttransferee*/