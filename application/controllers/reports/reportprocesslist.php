...
class ReportProcessList extends CI_Controller {
  ...
  function __construct()
  {
    ...
    $this->core->checkUserAllows(REPORTPROCLIST_NO);//REPORTPROCLIST_NO);
  ...
  function index()
  {
    $this->load->view('reports/reportprocesslist');
  }
  ...
  function getData()
  {
    ...
    foreach($replist as $row) {
      ...
      if (($userid == $this->core->getUserID() && $xml->getVALUE('ReportID') != 66) || $xml->getVALUE('ReportID') == 9999 ) {
        ...
        if (in_array($xml->getVALUE('ReportID'), array(39,40) )) {
          $fileType = '.CSV';
        } else if (in_array($xml->getVALUE('ReportID'), array(34,37) )) {
          $fileType = '.zip';
        } else if ($xml->getVALUE('ReportID') == 9999) {
          switch (intval($this->core->getUserGroup())) {
            case 1:
              $fileType = 'CSV'.date('mdY', strtotime($row['DateRequested'])).'.zip';
              break;
            case 2:
              $fileType = 'PDF'.date('mdY', strtotime($row['DateRequested'])).'.zip';
              $fileTypex = 'FILE'.date('mdY', strtotime($row['DateRequested'])).'.zip';
              $isbnkadmn = TRUE;
              break;
            default:
              $continue = TRUE;
        ...
        if (intval($row['ReportStatus']) == 3){
          $href = '<a id="xmlLink" href="'.$https.'://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/pdf/'.$xml->getVALUE('fileName').$fileType.'" target="_blank">For Download</a>';
          $hrefx = '<a id="xmlLink" href="'.$https.'://'.$_SERVER['SERVER_NAME'].':'.$_SERVER['SERVER_PORT'].'/pdf/'.$xml->getVALUE('fileName').$fileTypex.'" target="_blank">For Download</a>';
        } else {
        ...
        if ($xml->getVALUE('ReportID') == 9999) {
          if (!$isbnkadmn) {
            $repname = $xml->getVALUE('fileName') . ' CSV';
          } else {
            $repname = $xml->getVALUE('fileName') . ' PDF';
          }
        }
