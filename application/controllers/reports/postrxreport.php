...
class POSTrxReport extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getPOSTransactionsPerBranch($reportLog, $branchCode, $dtFrom, $dtTo);
		...
		foreach ($tpt as $row) {
			$xml->setXML($row['xml1'] . $row['xml2'] . $row['xml3'] . $row['xml4'] . $row['xml5'] . $row['xml6'] . $row['xml7']);
			...
			if (!array_key_exists($row['brcode'], $data)) {
				$data[$row['brcode']] = NULL;
			}
			...
			$content = '<tr>
				...
				<td width="230">'. $traceNo .'</td>
				<td width="230">'. $sequenceNo .'</td>
				<td width="150">'. $core->formatDate('H:i:s', $row['dtlog']) .'</td>
				<td width="150">'. $core->formatDate('m/d/y', $row['dtlog']) .'</td>
				<td width="120">'. $row['tpluno'] .'</td>
				<td width="340">'. substr($row['prkey1'], 0, 6).'- '.substr($row['prkey1'], 6).'</td>
				<td width="90" align="center">'. $row['msgtype'] .'</td>	
				<td width="210">'. $mnemonic .'</td>	
				<td width="300" '. $a1Align .'>'. substr($acct1, -14) .'</td>
				<td width="90" align="center">'. $acctDesc .'</td>
				<td width="110">'. $row['sysvcode'] .'</td>	
				<td width="150" >'. $mercID .'</td>	
				<td width="320" '. $rAlign .'>'. $remarks .'</td>		
				<td width="320" align="center">'. $acqCode .'</td>		
			...
			switch ($row['trxtype1']) {
				case 'WDL':
					//$wdl += $addAmt;
					$data[$row['brcode']]['wdl'] += $addAmt;
					break;
				case 'TRN':
					//$trn += $addAmt;
					$data[$row['brcode']]['trn'] += $addAmt;
					break;
				case 'PAY':
					//$pay += $addAmt;
					$data[$row['brcode']]['pay'] += $addAmt;
					break;
				case 'DEP':
					//$dep += $addAmt;
					$data[$row['brcode']]['dep'] += $addAmt;
					break;
				case 'ADV':
					//$adv += $addAmt;
					$data[$row['brcode']]['adv'] += $addAmt;
					break;
				case 'POS':
					//$adv += $addAmt;
					$data[$row['brcode']]['pos'] += $addAmt;
					break;
				case 'IBFT':
					switch ($mnemonic) {
						case 'IBFTSA':
						...
						case 'IBFTBTCA':
							//$ibftreq += $addAmt;
							$data[$row['brcode']]['ibftreq'] += $addAmt;
							break;
						case 'IBFTWDSA':
						case 'IBFTWDCA':
							//$ibftwdl += $addAmt;
							$data[$row['brcode']]['ibftwdl'] += $addAmt;
							break;
						case 'IBFTTRSA':
						case 'IBFTTRCA':
							//$ibfttrn += $addAmt;
							$data[$row['brcode']]['ibfttrn'] += $addAmt;
							break;
					}
					//rules for ELD 8882 issue
					if (intval($xml->getValue('IBFTPTR')) === IBFTPTR) {
			...
			$data[$row['brcode']]['content'] .= $content;
		}
		...
		$pdf->SetTitle('POS Transaction Report');
		$pdf->SetSubject(NULL);
		...
		foreach ($data as $key => $d) {
			...
			<h2>'.$instName.'</h2>
			<h1>POS TRANSACTION REPORT : '.$key.'</h1>
			<h3>'. $branchName .'</h3>
			...
			<thead>
				<tr>
					...
					<th width="230">TRACE NO.</th>
					<th width="230">SEQUENCE NO.</th>
					<th width="150">TIME</th>
					<th width="150">DATE</th>
					<th width="120">LUNO</th>
					<th width="340">BANK ID / CARD #</th>
					<th width="90" align="center">TC</th>
					<th width="210">TRXCODE</th>
					<th width="300" align="center">ACCOUNT</th>
					<th width="90" align="center">TY</th>
					<th width="110" align="center">VCD</th>
					<th width="150" align="center">MERCID</th>
					<th width="320" align="center">REMARKS</th>
					<th width="320" align="center">ACQUIRER</th>
			...
			<thead>
				<tr align="center">
					...
					<th align="right">POS PURCHASE</th>
		...
		$pdf->Output('postrxreport_' .$dateSaveFormat . '.pdf', 'I');
	}
}
