...
class LocEdit extends CI_Controller {
  ...
  function index()
  {
    ...
    if ($core->isHeadOffice()) {
      ...
      if ($result->num_rows() > 0) {  
        foreach ($result->result_array() as $row) {
          $selected = $location['brCode'] === $row['brcode'] ? ' selected' : NULL;
          $data['brInput'] .= '<option value="'. $row['brcode'] .'"'. $selected .'>'. $row['brname'] .'</option>';
        }
      } else {
    ...
    $auditXML .= '<old></><new></><field>Location Info</><details>'.$location['locName'].'</>';
  ...
  function submit()
  {
    ...
    if ($row['errno'] !== '8') { //if session valid
      ...
      if ($row['errno'] > 0) {
        ...
        if ($location = $this->cache->get($this->core->getSessionID() . 'location')) {
          if ($location['locName'] != $this->input->post('location', TRUE)) {
            $auditXML = '<old>'.$location['locName'].'</><new>'.$this->input->post('location', TRUE).'</><field>Location Name</><details>Location: '.$location['locName'].'</>';
            $this->card_model->insertAuditLogclixx(41,'990317',$brseqno,0,0,'LOC','',$userAudit,'','','','','','','',$userAudit,'','WEB','WEB',$workstation,$auditXML);
          }
        }
      }
    } else {
