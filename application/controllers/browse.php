...
class Browse extends CI_Controller {
	...
	function index()
	{
		...
		$this->cache->clean();
		//card status
		if (!$cardStatus = $this->cache->get('cardStatus')) {			
			...
			$this->cache->save('cardStatus', $cardStatus, CACHE_TTL);
		}
		...
		//card types
		if (!$cardType = $this->cache->get('cardType')) {
			...
			$this->cache->save('cardType', $cardType, CACHE_TTL);
		}
	...
	function getData()
	{
		...
		$sWhere = "WHERE (a.prtype = 'CARD' AND a.brseqno = ".$brseqno . $custom;//FRANZ
		if ( $_GET['sSearch'] !== '' )
		{
			$sWhere .= ") AND (";
		...
		$sQuery = "SELECT COUNT(".$sIndexColumn.") as count".
			" FROM ". $sTable ." WHERE (prtype = 'CARD' AND brseqno = ". $brseqno .")";
		...
		foreach ($rResult->result_array() as $aRow) {
			...
			foreach ($cardType as $stat) {
				$cardTypeDesc = $aRow['accttype'] === $stat['accttype'] ? $stat['description'] : $cardTypeDesc;
			}
			
			//card status
			$cardStatDesc = 'Unknown';
			foreach ($cardStatus as $stat) {
				$cardStatDesc = $aRow['status'] === $stat['status'] ? $stat['description'] : $cardStatDesc;
			}
	...
	function cache()
	{
		...
		$result = $card->getCardInfo($cardNo, $branchID, $ipAddress, $workstation, $userAudit);
