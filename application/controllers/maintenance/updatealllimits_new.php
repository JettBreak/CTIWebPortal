...
class UpdateAllLimits extends CI_Controller {
	...
	function submit()
	{
		...
		$params = array($accttype,$limitseqno);
		...
		$sql = "SELECT a.prseqno, a.brseqno ".
			   ...
			   "WHERE a.prseqno = b.prseqno AND ".$brseqnoSQL." accttype = ? AND limitseqno = ? ".
			   "GROUP BY 1 ";
		...
		for ($i = 0; $i < $numRows; $i += $limit) {
			...
			foreach ($lastResult as $row) { //loop all queried cards ------------------------------------------------------------------- foreach
				...
				foreach ($transaction as $index => $trxcode) { //loop all transactions --------------------------------------------------------- foreach
					...
					//init var
					$defaults = array('cyclemax' => 0,'nonfeetranmax' => 0,'ctrmax' => 0,'nonfeectrmax' => 0,'tranmin' => 0,'tranmax' => 0,'nonfeecycle' => 0,'duralimit' => 0,'cycle' => 0);
					
					//checking of checkboxes ---------------------------------------------------------------------------------------------- if
					if ( isset($amt[$index]) || isset($ctr[$index]) || isset($min[$index]) || isset($max[$index]) || isset($cyc[$index]) ) {
					...
					if ($brseqno != $limitbrseqno) { //------------------------------------------------------------------------------------ if
						$sql = "DELETE FROM limitxxx WHERE prseqno = ? AND trxcode = ? LIMIT 1";
						$params = array($prseqno, $trxcode);
						$card->db->query($sql, $params);
						
						$sql = "INSERT INTO limitxxx ( ".
							   "prseqno, trxcode, limitseqno, cycleavail, cyclemax, ctravail, ctrmax, tranmax, ".
							   "tranmin, duralimit, cycle, nonfeectravail, nonfeectrmax, nonfeetranavail, ".
							   "nonfeetranmax, useraudit, override, wkstn) ".
							   "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
						...
						$sql = "UPDATE limitxxx SET ". implode(', ', $set) ." WHERE prseqno = ? AND limitseqno = ? AND trxcode = ? LIMIT 1";
					...
					if ($r === 0) { //----------------------------------------------------------------------------------------------- if
						$sql = "SELECT prseqno FROM limitxxx WHERE prseqno = ? AND limitseqno = ? AND trxcode = ?";
						$params = array($prseqno, $limitseqno, $trxcode);
						
						$result = $card->db->query($sql, $params);

						if ($result->num_rows() === 0) {
							$sql = "DELETE FROM limitxxx WHERE prseqno = ? AND trxcode = ?";
							$params = array($prseqno, $trxcode);
							$card->db->query($sql, $params);
				...
				if ($totalProg !== round($progress/($numRows * count($transaction)) * 100)) {
					$totalProg = round($progress/($numRows * count($transaction)) * 100);
