...
<div id="bottom">
	<span class="buttons floatLeft">
    	<button id="generateBtn" value="card/generation/generate" disabled>Generate</button
        >
	</span>
...
$(function() {
	...
	oTable = $(DATATABLE).dataTable({
		...
		fnRowCallback: function(nRow, aData, iDisplayIndex) {
			$('td:eq(0), td:eq(1), td:eq(4), td:eq(5), td:eq(6), td:eq(8)', nRow).attr('align', 'center');
...
function updateProgress(orderNo, progress, ctr)
{
	$(orderNo).find('td:eq(6)').text(ctr); 
	$(orderNo).find('td:eq(7)').text('PROCESSING'); 
	$(orderNo).find('td:eq(8)').text(progress + '%');
}
