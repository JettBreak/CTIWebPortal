...
class DailySettlementPerBank extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getSettlementList($reportLog, $branchCode, $termType, $finswitch, $dtFrom, $dtTo);
		...
		foreach ($tpt as $row) {
			...
			$amtAth = $row['amtath'];
			$fee = $row['fee1'];			
			$data[$row['isscode']]['description'] = $description;
			
			//echo '<pre>' .print_r($row). '</pre>';
			if ($row['termtype'] === 'ATM') {
				$data[$row['isscode']]['AmtAthAcq'] += $amtAth;
				$data[$row['isscode']]['FeeAcq'] += $fee;
				$data[$row['isscode']]['cntrAcq']++;
				$totals['TotalAmtAcq'] += $amtAth;
				$totals['TotalNumAcq'] ++;
				$totals['TotalFeeAcq'] += $fee;
			} else {
				$data[$row['isscode']]['AmtAthIss'] += $amtAth;
				$data[$row['isscode']]['FeeIss'] += $fee;
				$data[$row['isscode']]['cntrIss']++;
				$totals['TotalAmtIss'] += $amtAth;
				$totals['TotalNumIss'] ++;
				$totals['TotalFeeIss'] += $fee;
			}
			//echo $amtAth.'<br>';
		}
		...
		$pdf->SetTitle('Daily Settlement Per Bank');
		$pdf->SetSubject(NULL);
		...
		//foreach ($data as $key => $d) {
			...
			<h2>'.$instName.'</h2>
			<h1>DAILY SETTLEMENT PER BANK:</h1>
			<h3>'. $branchName .'</h3>
			<h4>Transaction Date(s): '. $reportDate .'</h4>
			<table cellspacing="12" width="100%">
			<thead>
				<tr>
					<th width="450" align="left">F.I. NAME</th>
			...
			<tfoot>
				<tr>
					...
					<td align="right">'.$core->currency($totals['TotalAmtIss']).'</td>
					<td align="right">'.$totals['TotalNumIss'].'</td>
					<td align="right">'.$core->currency($totals['TotalFeeIss']).'</td>
					<td align="right">'.$core->currency($totals['TotalAmtAcq']).'</td>
					<td align="right">'.$totals['TotalNumAcq'].'</td>
					<td align="right">'.$core->currency($totals['TotalFeeAcq']).'</td>
					<td align="right">'.$totals['TotalNet'].'</td>
				</tr>
			</tfoot>
			</table>';
		...
		$pdf->Output('dailysettlementperbank_' .$dateSaveFormat . '.pdf', 'I');
	}
}
/* End of file dailysettlementperbank.php */
/* Location: ./application/reports/dailysettlementperbank.php */