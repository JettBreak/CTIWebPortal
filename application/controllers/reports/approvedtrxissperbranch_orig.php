...
class ApprovedTrxIssPerBranch extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getSumIssuerPerBranch($reportLog, $branchCode, $termType, $finswitch, $dtFrom, $dtTo);
		...
		foreach ($tpt as $row) {
			...
			if ( !array_key_exists($brCode, $branch) ) {
				continue;
			}
			
			switch ($row['trxtype1']) {
				case 'IBFT':
					$branch[$brCode]['ibft'] += intval($row['totctr']);
					break;
				case 'INQ':
					$branch[$brCode]['balInq'] += intval($row['totctr']);
					break;
				case 'WDL':
						$branch[$brCode]['wdrls'] += intval($row['totctr']);
						$branch[$brCode]['wdlAmt'] += floatval($row['totamt']);
					break;
				case 'REV':
					$branch[$brCode]['rvctr'] += intval($row['totctr']);
					$branch[$brCode]['wdlRversl'] += floatval($row['totamt']);
					break;
				case 'TRN':
					$branch[$brCode]['ftrn'] += intval($row['totctr']);
					break;
				case 'ADV':
					$branch[$brCode]['advcs'] += intval($row['totctr']);
					$branch[$brCode]['advAmt'] += floatval($row['totamt']);
					break;
				case 'DEP':
					$branch[$brCode]['deps'] += intval($row['totctr']);
					break;
				case 'STAT':
					$branch[$brCode]['stmr'] += intval($row['totctr']);
					break;
				case 'CKBK':
					$branch[$brCode]['chkr'] += intval($row['totctr']);
					break;
				case 'PAY':
					$branch[$brCode]['paym'] += intval($row['totctr']);
					break;
				case 'LOAD':
					$branch[$brCode]['load'] += intval($row['totctr']);
					break;
				default:
					break;
			}
			
			$branch[$brCode]['reserve2'] += 0;
		}
		...
		foreach ($branch as $code => $r) {
			...
			$data .= '<tr>'.
				'<td align="left"  width="300">'. $code . '-'. $r['brname'] .'</td>'.//branch
				...
				'<td align="right">'. $core->currency($load) .'</td>'.//reserve1 '<td align="right">'. $core->currency($reserve2) .'</td>'.//reserve2
			'</tr>';
		...
		$pdf->SetTitle('Approved Transactions, Issuer Per Branch Summary');
		$pdf->SetSubject('');
		...
		<h2>'.$instName.'</h2>
		<h1>APPROVED TRANSACTIONS, ISSUER PER BRANCH SUMMARY</h1>
		<h3>'. $branchName .'</h3>
		...
		<table cellspacing="18" width="100%">
			...
			<tfoot>
				<tr>
					...
					<td align="right">'. $core->currency($totals['load']) .'</td>
				</tr>
			</tfoot>
		</table>';
		...
		$pdf->Output('approvedtrxissperbranch_' . $dateSaveFormat . '.pdf', 'I');
	}
}
/* End of file approvedtrxissperbranch.php */
/* Location: ./application/reports/approvedtrxissperbranch.php */