...
class Barts extends CI_Controller {	
    ...
    function process()
    {
        ...
        foreach ($rfb as $row) {
			...
			if ($stISS === 0) {
				$stISS = trim(substr($row['prkey1'], 0, 6));	
				
				$result->free_result();
				$result->next_result();
				
				$result = $this->reports_model->getIssoCode($stISS);
            ...
            if (! ( ($stType === 'ATM') && in_array($stNode, array('ONUS', $onusCode)) ) ) {
                ...
                if ($stTran !== 'ERR') {
					...
					if ( /*($stTBCode !== $bnkcode) &&*/ in_array($stTran, array('IBF', 'IBC', 'TFR', 'TFC')) && in_array($stMnemonic, array('IBFTTRCA', 'IBFTTRSA')) ) {
					  //$type = 'TFREE';
