...
class AccountAdd extends CI_Controller {
		...
		//if ( in_array(array('acctno' => $accntNo), $_SESSION['accounts'][$key]['acctno']) ) {
			...
			if ($result->num_rows() > 0) {
				...
				if ($_SESSION['cardInfo']['brseqno'] !== $row['brseqno']) { //check branch
					...
					if ($hasPrimary) {
						$primary = count($_SESSION['accounts']) === 0 ? 'Y' : $primary;
					} else {
						$primary = 'Y';
					}
	...
	function link()
	{
		...
		if (in_array($accntNo, $_SESSION['accounts']) === TRUE) {
			...
			$sacntr = 0;
			$cacntr = 0;
			foreach ($_SESSION['accounts'] as $rowprptr) {
				$xmlX->setXML($rowprptr['xml1']);

				$accTypeX	= $xmlX->getValue('ACCT');
			...
			if ($sacntr > 0 || $cacntr > 0) {
				foreach ($_SESSION['accounts'] as $rowprptr) {
					...
					if ($accTypeX === 'CA') {
						$caArrPtr++;
						$caArray[] = array(
							'pseqnolink' => $rowprptr['pseqnolink'],
							'prptr' => $caArrPtr
						);
					}
				}
			...
			if ($accntCode === 'SA') {
				...
				foreach ($caArray as $caRow) {
					# code...
					$pseqnoLinkX = $caRow['pseqnolink'];
					$prptr = $caRow['prptr'] + 1;
					
					//$xml = '';
					$isSetprimary = 0;

					$result = $card->updateCardAccountLink(
						$prseqno,
						$pseqnoLinkX,
						$prptr,
						$xml,
