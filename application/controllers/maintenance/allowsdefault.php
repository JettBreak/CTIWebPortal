...
class AllowsDefault extends CI_Controller {
  ...
  function index()
  {
    ...
    if (count($productTypes) > 0) {
      foreach ($productTypes as $row) {
        ...
        foreach ($result->result_array() as $row) {
          ...
          //$checked = NULL;
          //$tranAllows .= '<label for="'. $prType .'bit'. $row['bitno'] .'"><input type="checkbox" id="'. $prType .'bit'. $row['bitno'] .'" name="'. $prType .'allows[]" value="'. $row['bitno'] .'"/>'. $row['description'] .'</label><br/>';
          $tranAllows .= '<label for="'. $prType .'bit'. $row['bitno'] .'"><input type="checkbox" id="'. $prType .'bit'. $row['bitno'] .'" name="'. $prType .'allows[]" value="'. $row['bitno'] .'"/>'. $row['description'] .'</label><br/>';
        }
        $tranAllows .= '</div>';
    ...
    if (count($accountTypes) > 0) {
      foreach ($accountTypes as $row) {
        ...
        if ($result->num_rows() > 0) {
          ...
          $defGreyed = rtrim($this->coreconverters->asciiHexToBin($row['allwview']), 0);
        } else {
          $defAllows = 0;
          $defGreyed = 0;
        }
        ...
        $acctTypes .= '<option value="'. $val .'" defaultallows="'. $defAllows .'" defaultgreyed="'. $defGreyed .'" grouptype="'. $groupType .'"'. $visibility .'>'. $desc .'</option>';
      }
    } else {
    ...
    if ($this->core->isISOCustomer()) {
      $data['prtypelbl'] = "Channel Type:";
      $data['accttypelbl'] = "Product Type:";
    } else {
      $data['prtypelbl'] = "Product Type:";
      $data['accttypelbl'] = "Account Type:";
    }
    //end
      ...
      $auditXML .= '<old></><new></><field>Allows Default List</><details>Open Module</>';
  ...
  function save()
  {
    ...
    $greyed = $this->input->post('greyed', TRUE);
    
    $allows = $this->coreconverters->asciiBinToHex($allows);
    $greyed = $this->coreconverters->asciiBinToHex($greyed);
