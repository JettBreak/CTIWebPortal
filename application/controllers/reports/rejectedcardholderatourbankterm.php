...
class RejectedCardHolderAtOurBankTerm extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getRejectedOurRChatAtOurTerminal($reportLog, $branchCode, $termType, $onusCode, $dtFrom, $dtTo);
		...
		foreach ($data as $key => $d) {
			...
			<h2>'.$instName.'</h2>
			<h1>REJECTED OUR CARDHOLDER AT OUR BANK TERMINAL: '. $key .'</h1>
			<h3>'. $branchName .'</h3>
		...
		$pdf->Output('rejectedcardholderatourbankterm_' .$dateSaveFormat . '.pdf', 'I');
	}
}
