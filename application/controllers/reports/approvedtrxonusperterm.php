...
class ApprovedTrxOnusPerTerm extends CI_Controller {	
	...
	function preview()
	{
		...
		$result = $reports->getSumOnUsPerTerminal($reportLog, $branchCode, $onusCode, $dtFrom, $dtTo);
		...
		$pdf->SetTitle('Approved Transactions, Onus Per Terminal Summary');
		$pdf->SetSubject('');
		...
		<h2>'.$instName.'</h2>
		<h1>APPROVED TRANSACTIONS, ONUS PER TERMINAL SUMMARY</h1>
		<h3>'. $branchName .'</h3>
		...
		$pdf->Output('approvedtrxonusperterm_' . $dateSaveFormat . '.pdf', 'I');
	}
}
/* End of file approvedtrxonusperterm.php */
/* Location: ./application/reports/approvedtrxonusperterm.php */