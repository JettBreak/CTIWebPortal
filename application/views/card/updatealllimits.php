...
class UpdateAllLimits extends CI_Controller {
	...
	function submit()
	{
		...
		$sql = "SELECT a.prseqno, a.brseqno "
			...
			."WHERE a.prseqno = b.prseqno AND accttype = ? AND limitseqno = ? "
			."GROUP BY 1 ";
		...
		if (isset($_POST['includeNon'])) { //if include product with no default limit is checked
			$sql .= "UNION ALL ".
				...
				"WHERE brseqno = ? AND accttype = ? AND prseqno NOT IN ".
		...
		$sql = "SELECT prseqno, brseqno FROM prmaster WHERE prkey = '5058130031080737'";
		...
		foreach ($transaction as $index => $trxcode) { //loop all transactions --------------------------------------------------------- foreach
			...
			foreach ($lastResult as $row) { //loop all queried cards ------------------------------------------------------------------- foreach
				...
				if ($brseqno != $limitbrseqno) { //------------------------------------------------------------------------------------ if
					...
					"SELECT ".
						"?, trxcode, limitseqno, ".
						"cycledef, cycledef, ctrdef, ctrdef, tranmaxdef, ".
						"tranmindef, duralimit, cycle, nonfeectrdef, nonfeectrdef, ".
						"nonfeetrandef, nonfeetrandef, ".
						"?, ?, ? FROM limitdef WHERE limitseqno = ? AND trxcode = ?)";
