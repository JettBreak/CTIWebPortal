...
class Info extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->core->checkUserAllows(CUSTINFO_NO);
    ...
    function index($id = NULL)
	{
		...
		if ($id === NULL || $core->isISOCustomer()) { //if no param set then fetch from cache
			...
			if ($cust = $this->cache->get($this->core->getSessionID() . 'cust')) {
				...
				$tbldata = $cust['tabledata'];
			} else {
				$this->load->helper('url');
				redirect('customer/search/info');
				exit();
			}
		}
		...
		if ($custappr === '') {
			...
			$data['onclose'] = $tbldata === 1 ? 'verify' : 'editcust';
			$result 	= $customer->searchCustomerApproval($id, '', '', '', $tbldata, $userAudit, $sessionID);	
		}
		...
		if ($result->num_rows() > 0) {
			...
			if ($core->isISOCustomer()) {
				$data['cifseqno'] = $custappr == TRUE ? $row['custkey'] : $row['cifseqno'];
			} else {
			...
			if ($core->isISOCustomer()) {
				if ($custappr !== '') {
					$data['custnum'] = 'CIF #:'.$row['custkey'];
				} else {
					$data['custnum'] = 'CIF #: '.$row['custkey'].'&nbsp;&nbsp;&nbsp; |&nbsp;&nbsp;&nbsp; Cust #: '.$row['cifseqno'];
				}
			} else {
				$data['custnum'] = 'CIF #: '.$row['cifseqno'];
			}
			...
			if ($custappr === '') {
				$result = $customer->getCustomerCardLink($id, $userAudit, $tbldata, $sessionID);
			...
			$data['type'] = 'info';
			if ($core->isISOCustomer()) {
				$data['title'] = '|'.$custappr.'|'.$id.'|'.$userAudit.'|'.$sessionID;
			} else {
				$data['title'] = 'Address';
			}
			$data['sessionExp'] = $this->core->getSessionExp();
